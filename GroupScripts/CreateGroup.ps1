...
try {
    ...
    if (-not $group) {
        # Ajout du groupe avec les bons attributs donnés en param
        New-ADGroup -Name $GroupName -Path $OrganisationUnit -GroupScope $GroupScope -Description $Description
        ...
        Write-Host "Le groupe '$GroupName' existe" -ForegroundColor Red
    }
} catch {
